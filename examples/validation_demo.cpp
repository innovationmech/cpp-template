...
void demonstrateBasicValidation() {
    ...
    std::cout << std::left << std::setw(25) << "Input" << std::setw(12) << "Empty?" << std::setw(15)
              << "Alphanumeric?" << std::setw(13) << "Valid Email?" << std::setw(20)
    ...
    for (const auto& test : test_cases) {
        ...
        std::cout << std::left << std::setw(25) << display_input << std::setw(12)
                  << (is_empty ? "Yes" : "No") << std::setw(15) << (is_alphanum ? "Yes" : "No")
                  << std::setw(13) << (is_email ? "Yes" : "No") << std::setw(20) << test.description
                  << std::endl;
    }
    std::cout << std::endl;
}
...
void demonstrateFormValidation() {
    ...
    for (size_t i = 0; i < test_forms.size(); ++i) {
        ...
        // Display results
        std::cout << "  Username: " << (form.username.empty() ? "(empty)" : form.username)
                  << std::endl;
        std::cout << "  Email: " << (form.email.empty() ? "(empty)" : form.email) << std::endl;
        std::cout << "  Password: " << (form.password.empty() ? "(empty)" : "[hidden]")
                  << std::endl;
...
void demonstrateDataCleaning() {
    ...
    std::vector<std::string> raw_data = {"  john@example.com  ",
                                         "JANE@EXAMPLE.COM",
                                         "",
                                         "   ",
                                         "invalid-email",
...
void demonstrateCustomValidation() {
    ...
    for (size_t i = 0; i < test_credentials.size(); ++i) {
        ...
        std::cout << "  Username validation: " << (username_valid ? "✓ Valid" : "✗ Invalid")
                  << std::endl;
        if (!username_valid) {
            std::cout << "    Error: " << username_error << std::endl;
        }

        std::cout << "  Password validation: " << (password_valid ? "✓ Valid" : "✗ Invalid")
                  << std::endl;
        if (!password_valid) {
        ...
        std::cout << "  Overall: "
                  << (overall_valid ? "✓ VALID CREDENTIALS" : "✗ INVALID CREDENTIALS") << std::endl;
        std::cout << std::endl;
    }
}
