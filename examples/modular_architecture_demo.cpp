...
void demonstrateBasicModularUsage() {
    ...
    try {
        ...
        std::vector<std::string> test_items = {"Hello World", "Modular Architecture",
                                               "C++ Template", "Data Processing"};
        ...
        for (const auto& item : test_items) {
            ...
            if (result.success) {
                ...
                std::cout << "   ✗ Error processing \"" << item << "\": " << result.error_message
                          << std::endl;
            }
        }
...
void demonstrateAdvancedProcessing() {
    ...
    try {
        ...
        std::vector<std::string> test_data = {"Simple text", "Complex data structure",
    ...
    try {
        ...
        std::vector<std::vector<std::string>> test_batches = {
            ...
            {"large1", "large2", "large3", "large4", "large5", "large6", "large7", "large8",
             "large9", "large10"}, // At limit
            {"exceed1", "exceed2", "exceed3", "exceed4", "exceed5", "exceed6", "exceed7", "exceed8",
             "exceed9", "exceed10", "exceed11"}  // Over limit
        };
        ...
        for (size_t i = 0; i < test_batches.size(); ++i) {
            ...
            auto result =
                processor->processBatch(batch, cpp_template::modules::ProcessingMode::BATCH);
...
void demonstrateConfigurationManagement() {
    ...
    try {
        ...
        auto result1 =
            processor->processItem("Initial test", cpp_template::modules::ProcessingMode::SIMPLE);
        if (result1.success) {
        ...
        auto result2 =
            processor->processItem("Updated test", cpp_template::modules::ProcessingMode::ADVANCED);
        if (result2.success) {
...
void demonstrateErrorHandling() {
    ...
    try {
        ...
        if (!result1.success) {
            std::cout << "  ✓ Correctly handled empty input: " << result1.error_message
                      << std::endl;
        }
        ...
        auto result2 =
            processor->processBatch(large_batch, cpp_template::modules::ProcessingMode::BATCH);
        if (!result2.success) {
            std::cout << "  ✓ Correctly handled batch size limit: " << result2.error_message
                      << std::endl;
        }
        ...
        auto result3 =
            processor->processItem("Valid input", cpp_template::modules::ProcessingMode::SIMPLE);
        if (result3.success) {
...
void demonstratePerformanceMonitoring() {
    ...
    try {
        ...
        std::cout << "Average time per item: "
                  << (duration.count() / static_cast<double>(performance_data.size())) << " ms"
                  << std::endl;
        ...
        auto batch_result =
            processor->processBatch(performance_data, cpp_template::modules::ProcessingMode::BATCH);
...
int main() {
    ...
    try {
        ...
        std::cout << "✓ All modular architecture demonstrations completed successfully!"
                  << std::endl;
