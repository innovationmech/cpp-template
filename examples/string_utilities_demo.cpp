...
void demonstrateCaseConversion() {
    ...
    std::cout << std::left << std::setw(25) << "Original" 
              << std::setw(25) << "Uppercase" 
    ...
    for (const auto& str : test_strings) {
        ...
        std::cout << std::left << std::setw(25) << display_str
                  << std::setw(25) << upper
...
void demonstrateSplitAndJoin() {
    ...
    for (const auto& test : test_cases) {
        ...
        std::cout << "Rejoined with \"" << test.join_delimiter << "\": \"" << rejoined << "\"" << std::endl;
        std::cout << std::endl;
    }
}
...
void demonstrateValidation() {
    ...
    std::cout << std::left << std::setw(20) << "String" 
              << std::setw(15) << "Empty?" 
              << std::setw(15) << "Alphanumeric?" 
              << std::setw(15) << "Valid Email?" 
              << std::setw(20) << "Description" << std::endl;
    std::cout << std::string(85, '-') << std::endl;
    
    for (const auto& test : test_strings) {
        std::string display_str = test.value.empty() ? "(empty)" : 
                                 (test.value.find_first_not_of(" \t\n\r") == std::string::npos ? "(whitespace)" : test.value);
        ...
        std::cout << std::left << std::setw(20) << display_str
                  << std::setw(15) << (is_empty ? "Yes" : "No")
                  << std::setw(15) << (is_alphanum ? "Yes" : "No")
                  << std::setw(15) << (is_email ? "Yes" : "No")
