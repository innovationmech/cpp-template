...
TEST_F(IntegrationTest, ConfigManagerDataProcessorIntegration) {
    ...
    EXPECT_TRUE(large_batch_result.error_message.find("exceeds configured limit") !=
                std::string::npos);
}
...
TEST_F(IntegrationTest, CompleteProcessingPipeline) {
    ...
    // Create test data
    std::vector<std::string> test_data = {"hello world", "integration test", "module interaction",
                                          "pipeline processing"};
    ...
    for (const auto& item : test_data) {
        ...
        auto processor_result =
            data_processor_->processItem(core_processed, ProcessingMode::ADVANCED);
        EXPECT_TRUE(processor_result.success);
...
TEST_F(IntegrationTest, ErrorHandlingIntegration) {
    ...
    EXPECT_THROW({ auto null_processor = createDataProcessor(nullptr); }, std::invalid_argument);
...
TEST_F(IntegrationTest, BuildExecutionPipeline) {
    ...
    std::vector<std::string> workflow_data = {"step1: initialization", "step2: configuration",
                                              "step3: processing", "step4: validation",
                                              "step5: completion"};
