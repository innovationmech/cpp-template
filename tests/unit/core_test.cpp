...
TEST_F(CoreTest, ConstructorWithEmptyName) {
    EXPECT_THROW({ Core core(empty_name_); }, std::invalid_argument);
}

TEST_F(CoreTest, ConstructorWithWhitespaceName) {
    EXPECT_THROW({ Core core(whitespace_name_); }, std::invalid_argument);
}
...
TEST_F(CoreTest, CreateCoreFactoryWithEmptyName) {
    EXPECT_THROW({ auto core_ptr = createCore(empty_name_); }, std::invalid_argument);
}
