...
int main() {
    std::cout << "=== " << cpp_template::info::getName() << " v" << cpp_template::info::getVersion()
              << " ===" << std::endl;
    ...
    try {
        ...
        std::cout << "'" << alphanumeric_str << "' is alphanumeric: "
                  << (cpp_template::validation::isAlphanumeric(alphanumeric_str) ? "true" : "false")
                  << std::endl;
        std::cout << "'" << email << "' is valid email: "
                  << (cpp_template::validation::isValidEmail(email) ? "true" : "false")
                  << std::endl;
        ...
        std::shared_ptr<cpp_template::modules::ConfigManager> config_manager(
            config_manager_unique.release());
        ...
        // Process single item
        auto result1 = data_processor->processItem("Hello Module",
        ...
        auto result2 =
            data_processor->processBatch(batch_data, cpp_template::modules::ProcessingMode::BATCH);
        if (result2.success) {
